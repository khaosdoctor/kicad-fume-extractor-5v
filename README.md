...
These renders are regenerated by CI on every push to `main`, so they always
match the latest board revision.

## Interactive BOM

The interactive HTML BOM is published via GitHub Pages:
[khaosdoctor.github.io/kicad-fume-extractor-5v/bom/ibom.html](https://khaosdoctor.github.io/kicad-fume-extractor-5v/bom/ibom.html)

That link only works once GitHub Pages is enabled for this repo (one-time
setup): Settings > Pages > Source: "Deploy from a branch" > Branch: `main`,
folder `/ (root)`. `bom/ibom.html` itself is refreshed by CI on every push
to `main`.
